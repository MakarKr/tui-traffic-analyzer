...
### MITM атаки
- ARP Spoofing для перехвата трафика между устройствами
- Автоматическое включение IP forwarding
- Восстановление ARP таблиц после остановки атаки
...
1. **Сканирование сети** -> `NetworkScanner`.
2. **Захват и анализ трафика** -> `PacketAnalyzer` + `SessionManager`.
3. **Атака MITM** -> `MITMAttacker`.
4. **Анализ SSL/TLS** -> `SSLAnalyzer`.
...
**Сделано с ❤️ для сетевой безопасности и обучения**
